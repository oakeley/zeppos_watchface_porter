...
export oX=480
export oY=480
#rat=`echo "480/416" | awk '{A=split($1,a,"/"); print a[1]/a[2]}'`
...
cat zeppos_watchdrip_timer_wf/watchface/gts4mini/styles.js | \
   sed "s/hour_startX: px(.*),/hour_startX: +("`echo 110 $xScale | awk '{print int(($1*$2)+.5)}'`"),/g" | \
...
cat /dev/shm/styles.js.tmp | \
   tr "(" "@" | \
   awk -v xS1=$xScale -v yS1=$yScale -F"px@" '{OFS="px@"; xS=xS1*.95; yS=yS1*.95; if ($0 ~ "px@") {A=split($2,a,")"); if (length(a[1])<4) {if ($0 ~ "x: px@" || $0 ~ "X: px@" || $0 ~ "w: px@") {x=int((a[1]*xS)+.5); $2=x")"a[2]}; if ($0 ~ "y: px@" || $0 ~ "Y: px@" || $0 ~ "h: px@") {y=int((a[1]*yS)+.5); $2=y")"a[2]}}}; print $0 }' | \
   tr "@" "(" | \
